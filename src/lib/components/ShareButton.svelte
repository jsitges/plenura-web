<script lang="ts">
	...
	const shareUrl = $derived(url || (browser ? window.location.href : $page.url.href));
	const shareText = $derived(text || `Mira el perfil de ${title} en Plenura`);
	const encodedUrl = $derived(encodeURIComponent(shareUrl));
	const encodedText = $derived(encodeURIComponent(shareText));

	const canNativeShare = $derived(browser && 'share' in navigator);
...
<div class="relative share-dropdown-container {className}">
	{#if variant === 'icon'}
		<button
			type="button"
			onclick={handleShare}
	...
	{:else if variant === 'compact'}
		<button
			type="button"
			onclick={handleShare}
			class="flex items-center gap-1.5 px-3 py-1.5 text-sm text-gray-600 hover:text-primary-600 hover:bg-gray-50 rounded-lg transition-colors"
		>
	...
	{:else}
		<button
			type="button"
			onclick={handleShare}
			class="flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors"
		>
	...
	{#if showDropdown}
		<div class="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg border border-gray-100 py-2 z-50">
			<p class="px-4 py-2 text-xs text-gray-500 uppercase tracking-wider">Compartir en</p>
			...
			>
				{#if copySuccess}
					...
					<span class="text-green-600 font-medium">Enlace copiado</span>
				{:else}
					<svg class="w-5 h-5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 012-2h2a2 2 0 012 2m0 0h2a2 2 0 012 2v3m2 4H10m0 0l3-3m-3 3l3 3" />
					</svg>
					<span class="text-gray-700">Copiar enlace</span>
				{/if}
			</button>
