...
<div class="max-w-lg mx-auto text-center space-y-6">
	...
	<div
		...
		class:bg-green-100={statusInfo.color === 'green'}
		class:bg-amber-100={statusInfo.color === 'amber'}
	>
		{#if statusInfo.icon === 'check'}
			<svg
				class="w-10 h-10"
				class:text-green-600={statusInfo.color === 'green'}
				fill="none"
				stroke="currentColor"
				viewBox="0 0 24 24"
			>
		...
		{:else}
			<svg
				class="w-10 h-10"
				class:text-amber-600={statusInfo.color === 'amber'}
				fill="none"
				stroke="currentColor"
				viewBox="0 0 24 24"
			>
	...
	<div class="bg-white rounded-xl border border-gray-100 p-6 text-left">
		<div class="flex items-center gap-4 mb-6">
			<div
				class="w-14 h-14 rounded-xl bg-primary-100 flex items-center justify-center flex-shrink-0"
			>
		...
		<div class="space-y-3">
			<div class="flex items-center gap-3">
				...
				<span class="text-gray-700">{formatDate(booking.scheduled_at)}</span>
			</div>
			...
			<div class="flex items-center gap-3">
				...
				<span class="text-gray-700">
					{formatTime(booking.scheduled_at)} - {formatTime(booking.scheduled_end_at)}
				</span>
			</div>
			...
			<div class="flex items-center gap-3">
				...
				<span class="text-gray-700">{booking.client_address}</span>
			</div>
		</div>
		...
		{:else if booking.status === 'confirmed'}
			<div class="mt-4 flex items-center gap-2 text-sm text-green-600 bg-green-50 rounded-lg p-3">
				...
				<span>Pago en escrow - se liberará al completar el servicio</span>
			</div>
		{/if}
	</div>
	...
	<div class="flex flex-col sm:flex-row gap-3">
		<a href="/bookings" class="flex-1 btn-primary-gradient py-3 text-center"> Ver mis citas </a>
		<a
			href="/therapists"
			class="flex-1 py-3 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors text-center"
		>
			Buscar más
		</a>
	</div>
	...
	<div class="bg-gray-50 rounded-xl p-6 text-left">
		<h3 class="font-semibold text-gray-900 mb-3">¿Qué sigue?</h3>
		<ol class="space-y-3 text-sm text-gray-600">
			{#if booking.status === 'pending'}
				<li class="flex items-start gap-3">
					<span
						class="w-6 h-6 bg-primary-100 text-primary-700 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold"
						>1</span
					>
					<span>Realiza el pago para confirmar tu cita</span>
				</li>
				<li class="flex items-start gap-3">
					<span
						class="w-6 h-6 bg-gray-100 text-gray-500 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold"
						>2</span
					>
					<span>Tu pago quedará protegido en escrow</span>
				</li>
				<li class="flex items-start gap-3">
					<span
						class="w-6 h-6 bg-gray-100 text-gray-500 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold"
						>3</span
					>
					<span>El terapeuta llegará a tu ubicación en la fecha acordada</span>
				</li>
			{:else}
				<li class="flex items-start gap-3">
					<span
						class="w-6 h-6 bg-green-100 text-green-700 rounded-full flex items-center justify-center flex-shrink-0"
					>
					...
					<span>Pago confirmado y protegido</span>
				</li>
				<li class="flex items-start gap-3">
					<span
						class="w-6 h-6 bg-primary-100 text-primary-700 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold"
				...
				<li class="flex items-start gap-3">
					<span
						...
						>3</span
					>
					<span>Al completar el servicio, el pago se liberará al terapeuta</span>
				</li>
			{/if}
		</ol>
	</div>
