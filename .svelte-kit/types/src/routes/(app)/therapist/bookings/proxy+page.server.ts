...
export const actions = {
	confirm: async ({ request, locals }: import('./$types').RequestEvent) => {
		const { supabase, user } = locals;
	...
	complete: async ({ request, locals }: import('./$types').RequestEvent) => {
		const { supabase, user } = locals;
		const formData = await request.formData();
		const bookingId = formData.get('bookingId') as string;
	...
	noshow: async ({ request, locals }: import('./$types').RequestEvent) => {
		const { supabase, user } = locals;
		const formData = await request.formData();
		const bookingId = formData.get('bookingId') as string;
