...
export const actions = {
	updateUser: async ({ request, locals }: import('./$types').RequestEvent) => {
	...
	updateTherapist: async ({ request, locals }: import('./$types').RequestEvent) => {
		const { supabase, user } = locals;
		const formData = await request.formData();
	...
	updateCertifications: async ({ request, locals }: import('./$types').RequestEvent) => {
		const { supabase, user } = locals;
		const formData = await request.formData();
